...
| Method | Path | Auth | Тайлбар |
|---|---|:---:|---|
| GET | `/reading` | 🔑 | Жагсаалт (`?cefr&category&search&page&limit`; оюутан зөвхөн published, админ `?all=true`) |
...
| POST | `/reading/:id/complete` | 🔑 | Уншиж дуусгах → нэг удаа +15 XP (idempotent). `{ alreadyCompleted, xpAwarded }` |
| POST | `/reading/guess-choices` | 🛡️ | **F1:** `{ words[], cefr? }` → AI 3 сонголттой "утга таах" массив (admin review, хадгалахгүй) |
| POST | `/reading/:id/generate-audio` | 🛡️ | **F4:** бүх өгүүлбэрт ElevenLabs audio background → `{ jobId }` |
| GET | `/reading/audio-job/:jobId` | 🛡️ | Audio job progress poll `{ total, processed, done }` |
| POST | `/reading/:id/sentences/:index/generate-audio` | 🛡️ | Нэг өгүүлбэрийн audio (дахин) үүсгэх |

> **Бүтэц:** `title`, `cefr` (ContentLevel), `category` (сэдэв — нээлттэй текст,
> `READING_CATEGORY_SUGGESTIONS`, mobile дээр таб болгон шүүнэ), `coverImageUrl`,
> `keyVocab` jsonb
...
## 💬 Idioms (Хэлц үг) — `/api/idioms`

Англи хэлц + монгол утга. Тусдаа модуль. Талбар: phrase, mongolian, meaning
(жинхэнэ утга), definition (тайлбар), example sentence+translation, image, audio.

| Method | Path | Auth | Тайлбар |
|---|---|:---:|---|
| GET | `/idioms` | 🔑 | Жагсаалт (`?search&page&limit&noImage`; оюутан published, админ `?all=true`) |
| GET | `/idioms/:id` | 🔑 | Нэг хэлц |
| POST | `/idioms` | 🛡️ | Үүсгэх |
| POST | `/idioms/ai-fill` | 🛡️ | `{ phrase }` → Gemini талбар бөглөнө (хадгалахгүй, admin review) |
| POST | `/idioms/ai-bulk` | 🛡️ | `{ phrases[], generateImages?, generateAudios? }` → AI бүх талбар бөглөж ноорог болгон нэмнэ (background) → `{ jobId }` |
| GET | `/idioms/ai-bulk/:jobId` | 🛡️ | AI-bulk job progress `{ total, processed, inserted, skipped, failed, done }` |
| POST | `/idioms/ai-bulk/:jobId/cancel` | 🛡️ | Ажиллаж буй AI-bulk job-ийг зогсоох → `{ canceled }` |
| POST | `/idioms/import` | 🛡️ | CSV (multipart, field `file`) → баталгаажуулалтын тайлан. Багана: phrase, mongolian (заавал) + meaning/definition/example.../imageUrl/audioUrl/isPublished |
| PATCH | `/idioms/bulk` | 🛡️ | `{ ids[], isPublished }` — олныг нэг дор нийтлэх/ноорог болгох |
| POST | `/idioms/bulk-generate-images` | 🛡️ | `{ ids[] }` → OpenAI зураг background → `{ jobId }` |
| GET | `/idioms/image-job/:jobId` | 🛡️ | Зургийн job progress `{ total, processed, done }` |
| PATCH | `/idioms/:id` | 🛡️ | Засах / publish toggle |
| DELETE | `/idioms/:id` | 🛡️ | Устгах |
| POST | `/idioms/:id/generate-audio` | 🛡️ | ElevenLabs дуудлага → `{ audioUrl }` (нэг удаа, кэш) |
| POST | `/idioms/:id/generate-image` | 🛡️ | OpenAI зураг → `{ imageUrl }` |
...
| Method | Path | Auth | Тайлбар |
|---|---|:---:|---|
| GET | `/dictionary/:word` | 🔑 | Богино монгол утга → `{ word, translation, audioUrl, cached }`. Дараалал: **Words DB → `translations` кэш → Gemini** (plan `dictionaryAiLimit`-тэй; Gemini хариуг кэшэлж дахин AI дуудахгүй). |
| GET | `/dictionary/:word/audio` | 🔑 | Дуудлагын аудио → `{ audioUrl }` (ElevenLabs). Эхний дуудлагад үүсгэж Cloudinary-д хадгална, дараа нь кэшээс. |
| POST | `/dictionary/:word/save` | 🔑 | Үгийг орчуулгатай нь хадгалсан үгсэд нэмэх → `{ wordId, saved }`. Words DB-д байхгүй бол `needs_review` Word үүсгээд WordReview.saved=true. |
