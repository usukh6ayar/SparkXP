...
import { parseBandTopic } from './ielts';
...
export function buildStepBrief(step: BulkStep, avoidTitles: string[]): string {
  /*
   * IELTS-д `topic` нь сэдэв БИШ, **зорилтот band** (ж: «Band 6.5»). Түүнийг
   * «Сэдэв: "Band 6.5"» гэж дамжуулбал загвар band-ын тухай эссэ бичих гээд
   * утгагүй болно — тиймээс хүндрэлийн зааварчилгаа болгож хөрвүүлнэ.
   */
  const band = parseBandTopic(step.topic);
  const lines = [
    `"${step.label}" ур чадварын дасгал.`,
    band !== null
      ? `ЗОРИЛТОТ BAND: ${band}. Үг хэллэг, өгүүлбэрийн бүтэц, сэдвийн ` +
        `хийсвэрлэлийг яг тэр band-ын шалгуулагчид тохирсон түвшинд бич — ` +
        `хэт хялбар ч, хэт хүнд ч биш. Сэдвээ өөрөө сонго.`
      : step.topic
        ? `Сэдэв: "${step.topic}".`
