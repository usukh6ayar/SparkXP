...
## 12. ✅ ШИЙДЭГДСЭН — DB latency (2026-07-29)

**Шалтгаан нь: Postgres ба Redis нь API-аас өөр бүсэд (US) байсан.**

`postgres.railway.internal` гэдэг нэр нь *дотоод сүлжээ* гэсэн үг л болохоос
*нэг бүсэд* гэсэн үг **биш**. Railway-гийн private network бүсүүдийг дамнадаг —
хаяг нь дотоод боловч пакет физикээр Сингапур ↔ US-West (~180 мс) явж байсан.

**Оношийг тогтоосон нотолгоо:** 6 хүсэлтийг дараалан явуулахад бүгд ижил удаан
байсан (560 / 467 / 489 / 467 / 515 / 489 мс). Хэрэв 180 мс нь *холболт үүсгэх*
зардал байсан бол зөвхөн эхнийх удаан байх ёстой (үлдсэн нь pool-ын халуун
холболтыг дахин ашиглана). Зургуулаа адилхан байсан нь **query бүр дээр
төлөгддөг цэвэр сүлжээний round trip** гэдгийг харуулсан.

**Засвар:** Railway → Postgres + Redis service → Settings → Region →
`asia-southeast1` (API-тай ижил).

### Үр дүн (deploy-ийн дараа хэмжсэн)

| Endpoint | Өмнө | Дараа | Сайжралт |
| --- | --- | --- | --- |
| `/api/__nope` (404, DB хүрэхгүй) | 292 мс | 288 мс | — *(цэвэр сүлжээ, өөрчлөгдөхгүй)* |
| `/api/lessons` | 470 мс | **291 мс** | **−38%** |
| `/api/words?limit=20` | 840 мс | **298 мс** | **−65%** |
| `/api/health` | 478 мс | **292 мс** | **−39%** |

**Query бүрийн нэмэлт зардал: 180 мс → ~2 мс.**

### Бодит апп-ын мэдрэх хурд (keep-alive холболт дээр)

Дээрх тоонууд `curl`-ийн хүсэлт тутам шинэ TLS холболт нээснийг агуулна. Mobile
апп нь холболтоо дахин ашигладаг тул бодит үзүүлэлт:

| Хүсэлт | Хугацаа | Үүнээс серверийн боловсруулалт |
| --- | --- | --- |
| Эхний (TLS-тэй) | 341 мс | — |
| `/api/lessons` | **105 мс** | ~12 мс |
| `/api/words?limit=20` | **156 мс** | ~63 мс (19 KB дамжуулалт орсон) |
| `/api/health` | **106 мс** | ~13 мс |

Үлдсэн ~93 мс нь Монгол ↔ Сингапурын физик зай — үүнээс доош буулгах боломжгүй.

### Хажуугаар хийсэн засвар — pool тохиргоо

`src/config/typeorm.config.ts`-д `min: 2`, `idleTimeoutMillis: 600_000`,
`keepAlive: true`, `max: 20`, `connectionTimeoutMillis: 5_000` нэмсэн (PR #190).
Энэ нь **үндсэн шалтгаан байгаагүй** — хэмжилтээр сул байсны дараах эхний
...
| --- | --- | --- | --- |
| 1 | ✅ **ШИЙДЭГДСЭН** — Postgres+Redis-ийг `asia-southeast1` руу зөөв (§12) | **840 мс → 298 мс**, query бүр 180 мс → ~2 мс | Dashboard, 10 мин |
